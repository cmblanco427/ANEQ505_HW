...
- Week 7 Tutorial: Beta Diversity, Longitudinal Analysis, Exporting Qiime2 Data
    ...
    - Exporting Qiime2 data
        - Homework 3
- Week 8: Reviewing the 16S workflow, Running Stats in R, Generating Publication-Quality Figures
    - Workflow overview
...
- Beta diversity stats for non-repeated and repeated measures

**This week:**

- Homework 3 review
- qiime2 workflow review
- Beta diversity stats on repeated measures, longitudinal data, Linear Mixed Effects (LME) models (in R)
...
Now that we have our data files in Alpine, we need to **import the data into a QIIME2 artifact**. Since it is **multiplexed, we don't need a manifest**, we just need to tell QIIME2 the name of the directory we just created and the type of data we are importing (which is **EMP paired end sequences**).

### **Demultiplexing**

**What is the difference between multiplexed and demultiplexed data? and what is the purpose of demultixplexing?**

When the data is multiplexed the samples are still pooled together in the fastq files. We need to demultiplex in order to distinguish between the different samples. This is possible due to the unique barcodes for each sample

### **Denoising with DADA2**

We will denoise using DADA2 to generate our amplicon sequence variants "ASVs".
...
after you classify your ASVs, a next step is commonly to filter the table. You can filter out chloroplasts and mitochondria, filter out rare features, or filter out samples (like controls).
