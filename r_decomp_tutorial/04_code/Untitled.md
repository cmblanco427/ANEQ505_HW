...
```{r}
# Load in diversity metrics that we downlaoded from Alpine
...
faiths_pd <- read_tsv("alpha_div/faith_pd.tsv") #sample name + alpha diversity number
```
...
```{r}
# Build model
model_add0_shannon <- lmer(shannon_entropy ~ add_0c + (1|host_subject_id), data = metadata) # I name all my models with the variable I am testing and the metric I am testing. lmer= linear mixed effects, shannon entropy=???m 
#to account for repeated measures have host_id

# Residual diagnostic plot, check assumptions
plot(model_add0_shannon) #show mdoel distribution, want it to be even across everything. 
# look at estimated variance components
summary(model_add0_shannon) #give variance, sd
# residual diagnostic plot, check assumptions
